...
export async function generateIdentity(name?: string): Promise<Identity> {
  ...
  return name !== undefined ? { pubkey, privateKey, name } : { pubkey, privateKey };
}

export async function loadIdentityFromFile(path: string): Promise<Identity> {
  const { readFileSync } = await import("node:fs");
  const raw = JSON.parse(readFileSync(path, "utf-8")) as { privateKey: string; name?: string };
  const privBytes = new Uint8Array(raw.privateKey.match(/.{2}/g)!.map((b) => parseInt(b, 16)));
...
export async function loadIdentityFromHex(hex: string, name?: string): Promise<Identity> {
  const privBytes = new Uint8Array(hex.match(/.{2}/g)!.map((b) => parseInt(b, 16)));
  const pubkey = await ed.getPublicKeyAsync(privBytes);
...
export async function sign(privateKey: Uint8Array, value: unknown): Promise<Signature> {
